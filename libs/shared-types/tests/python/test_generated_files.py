...
def test_file_content(filename, file_path):
    ...
    # Check for either full generation or CI stub
    is_ci_stub = "Minimal stub for CI" in content
    is_full_generation = ("from_dict" in content and "to_dict" in content) or "BaseModel" in content

    if not is_ci_stub and not is_full_generation:
        raise Exception(f"File {filename} doesn't appear to be properly generated (missing expected patterns)")

    if is_ci_stub:
        print(f"✓ File content valid (CI stub): {filename}")
    else:
        print(f"✓ File content valid (full generation): {filename}")
...
def test_imports(filename, file_path):
    ...
    try:
        ...
        # Check if this is a CI stub or full generation
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()

        is_ci_stub = "Minimal stub for CI" in content
        if is_ci_stub:
            print(f"✓ Module imports correctly (CI stub): {filename}")
        else:
            print(f"✓ Module imports correctly (full generation): {filename}")
