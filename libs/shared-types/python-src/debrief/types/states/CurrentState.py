...
class EditorState:
    """Complete editor state containing all sub-states
...
class CurrentState:
    ...
    filename: str
    """Filename of the document being edited"""

    history_count: int
    """History count for this editor"""

    def __init__(self, editor_id: str, editor_state: EditorState, filename: str, history_count: int) -> None:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'CurrentState':
        assert isinstance(obj, dict)
        editor_id = from_str(obj.get("editorId"))
        editor_state = EditorState.from_dict(obj.get("editorState"))
        filename = from_str(obj.get("filename"))
        history_count = from_int(obj.get("historyCount"))
        return CurrentState(editor_id, editor_state, filename, history_count)

    def to_dict(self) -> dict:
        result: dict = {}
        result["editorId"] = from_str(self.editor_id)
        result["editorState"] = to_class(EditorState, self.editor_state)
        result["filename"] = from_str(self.filename)
        result["historyCount"] = from_int(self.history_count)
        return result


def current_state_from_dict(s: Any) -> CurrentState:
    return CurrentState.from_dict(s)


def current_state_to_dict(x: CurrentState) -> Any:
    return to_class(CurrentState, x)
