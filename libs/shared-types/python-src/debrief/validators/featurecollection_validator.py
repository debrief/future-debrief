...
try:
    from ..types.featurecollection import DebriefFeatureCollection
except ImportError:
    # If import fails, define a minimal type for validation
    class DebriefFeatureCollection:  # type: ignore
        pass
...
def validate_bbox(bbox: List[float]) -> bool:
    ...
    if not all(
        isinstance(val, (int, float)) and not (val != val or val == float("inf") or val == float("-inf"))
        for val in bbox
    ):
        return False
...
def get_feature_counts(collection: Dict[str, Any]) -> Dict[str, int]:
    ...
    """
    features = collection.get("features", [])
    counts = {"tracks": 0, "points": 0, "annotations": 0, "unknown": 0, "total": len(features)}
...
def validate_feature_collection_comprehensive(collection: Any) -> Dict[str, Any]:
    """
